...
mi <- 3    # Number of incomplete variables
...
dat <- simulate.scenario.shared(K = k, n = n, p = p, s = s, v = v, baseline_params = list(E0), h = h, eta = 0, cluster_sizes = c(k, 0), simulate_clusters = F, complete_data_type = c(norm = 1, binom = 0), incomplete_data_type = c(norm = 1, binom = 0))

# Extract the data for each task and form a new list
data <- lapply(dat, function(x) x$data)

# simulate the missing at random
dat.mar <- mar.sim(data = data, covaraite = "c_x1", where.missing = c("ic_x1","ic_x2","ic_x3"), prop.missing = 0.2)
...
file.name <- paste0("/scratch/yc4178/MTL-MICE/sim/output/htrends/h_1se_h", h, "_", task.id, ".RData")
