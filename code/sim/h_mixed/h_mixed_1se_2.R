...
dat <- simulate.scenario.shared(K = k, n = n, p = p, s = s, v = v, baseline_params = list(E0), h = h, eta = 0, cluster_sizes = c(k, 0), simulate_clusters = F, complete_data_type = c(norm = 1, binom = 0), incomplete_data_type = c(norm = 0.5, binom = 0.5))
...
file.name <- paste0("/scratch/yc4178/MTL-MICE/sim/output/htrends/h_mixed_1se_h", h, "_", task.id, ".RData")
